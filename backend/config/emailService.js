...
const getFrontendUrl = () =>
  ...
  (process.env.NODE_ENV === 'production'
    ? 'https://expense-tracker-cicd-pearl.vercel.app'
    : 'http://localhost:5173');
...
const verificationTemplate = (link) => ({
  ...
  html: `
    <div style="font-family: Arial, sans-serif; max-width:600px; margin:auto;">
      ...
      <p>Click the button to verify your email. The link expires in 24 hours.</p>
      <p style="text-align:center">
        <a href="${link}" style="background:#10b981;color:#fff;
           padding:14px 28px;border-radius:8px;text-decoration:none;">Verify Email</a>
      </p>
      <p>If the button fails copy this link: <a href="${link}">${link}</a></p>
    </div>`
});
...
const resetTemplate = (link) => ({
  ...
  html: `
    <div style="font-family: Arial, sans-serif; max-width:600px; margin:auto;">
      ...
      <p>Click below to set a new password. The link expires in one hour.</p>
      <p style="text-align:center">
        <a href="${link}" style="background:#10b981;color:#fff;
           padding:14px 28px;border-radius:8px;text-decoration:none;">Reset Password</a>
      </p>
      <p>If the button fails copy this link: <a href="${link}">${link}</a></p>
    </div>`
});
...
const sendVerificationEmail = (email, token) => {
  ...
  return sendEmail({ to: email, ...verificationTemplate(link) });
};

const sendPasswordResetEmail = (email, token) => {
  const link = `${getFrontendUrl()}/reset-password/${token}`;
  return sendEmail({ to: email, ...resetTemplate(link) });
};
