...
const IncomeList = ({ 
  ...
  const virtualizedTransactions = useMemo(() => {
    return filteredTransactions.map(transaction => ({
      ...transaction,
      type: 'income',
      source: transaction.source, // Ensure source field is available
      category: transaction.source // Map source to category for consistency
    }));
  ...
  // Enhanced Income Card Component (for regular grid view)
  const IncomeCard = ({ transaction }) => {
    const renderIcon = () => {
      if (transaction.icon) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div 
              ...
              style={{ 
                backgroundColor: '#ecfdf5', // green-50 equivalent
            ...
            <div>
              <h6 className="text-sm font-medium text-gray-800">{transaction.source}</h6>
  ...
  return (
    <div className="card">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {getPeriodText()} • {filteredTransactions.length} income sources • ${totalAmount.toFixed(2)} total
          </p>
        </div>
                ...
                <div className="py-1">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        All time • Complete income history
                      </div>
                    </div>
                  </button>
                </div>
      ...
      {/* Income Display - Virtual Scrolling for Large Datasets */}
        ...
        shouldUseVirtualScrolling ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filteredTransactions.map((transaction) => (
              <IncomeCard key={transaction._id} transaction={transaction} />
            ))}
          </div>
        )
      ) : (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            {viewMode === 'monthly' || viewMode === 'annual'
              ? `No income recorded for ${getPeriodText()}`
